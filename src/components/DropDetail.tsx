...
export function DropDetail({ drop, onBack }: DropDetailProps) {
  const progress = drop.tasks_count > 0
    ? Math.round((drop.completedTasks / drop.tasks_count) * 100)
    : 0;

  // Задачи: из API (tasks_list) или сгенерированные
  const tasks = drop.tasks_list && drop.tasks_list.length > 0
    ? drop.tasks_list
      ...
      <div className="flex flex-col gap-4 px-4 pb-nav pt-4">
        ...
        {drop.tasks_count > 0 && (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <p className="text-[12px] font-extrabold" style={{ color: drop.gradientFrom }}>
                {progress}%
              </p>
            </div>
            <div className="h-2.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}>
              <div
                ...
                style={{
                  ...
                  boxShadow: `0 0 10px ${drop.glowColor}`,
                }}
              />
            </div>
            <p className="text-[10px] text-zinc-600 mt-1.5">
              {drop.completedTasks} of {drop.tasks_count} tasks completed
            </p>
          </div>
        )}
          ...
          <div className="space-y-2">
            {tasks.map((task: any, i: number) => (
              <div
                ...
                style={{
                  background: task.completed ? "rgba(16,185,129,0.06)" : "rgba(255,255,255,0.03)",
                  border: `1px solid ${task.completed ? "rgba(16,185,129,0.15)" : "rgba(255,255,255,0.07)"}`,
                }}
              >
                {task.completed ? (
                ...
                <div className="flex-1 min-w-0">
                  <p
                    ...
                    style={{
                      color: task.completed ? "#6ee7b7" : "#e4e4e7",
        ...
        {drop.project_url && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="relative overflow-hidden flex items-center justify-center gap-2 rounded-2xl py-3.5 text-[14px] font-bold text-white transition-all active:scale-[0.97]"
            style={{
              background: `linear-gradient(135deg, ${drop.gradientFrom}, ${drop.gradientTo})`,
        ...
        >
          ...
          <p className="text-[11px] text-zinc-400 leading-relaxed">
            ...
            Complete 1-2 tasks per day with randomized amounts for maximum organic score.
          </p>
        </div>
...
/* ── Fallback tasks if API has none ──── */
function generateFallbackTasks(drop: any) {
  ...
  return templates.slice(0, count).map((t, i) => ({
    ...
    description: "",
    completed: i < (drop.completedTasks || 0),
  }));
}